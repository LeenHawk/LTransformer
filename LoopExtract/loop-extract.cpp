...
static llvm::cl::opt<unsigned int> MaxLoopLayerNumber("N", llvm::cl::desc("The biggest number of layer recognized"),
                                                      llvm::cl::value_desc("MaxLayerNumber"), llvm::cl::init(1),
                                                      llvm::cl::cat(ToolingSampleCategory));
...
public:
    unsigned int getInnerLoopLayerNumber(ForStmt *forStmt) {
        ...
        for (Stmt *subStmt: forStmt->getBody()->children()) {
            if (isa<ForStmt>(subStmt)) {
                maxInnerLoopLayerNumber = std::max(getInnerLoopLayerNumber(dyn_cast<ForStmt>(subStmt)),
                                                   maxInnerLoopLayerNumber);
            }
        }
    ...
    bool VisitDeclRefExpr(DeclRefExpr *declRefExpr) {
        if (isValueSearch) {
            ...
            if (std::find(innerInitialParams.begin(), innerInitialParams.end(), std::make_pair(type, name)) ==
                innerInitialParams.end())
    ...
    bool VisitForStmt(ForStmt *forStmt) {
        ...
        if (LoopNumber <= MaxLoopLayerNumber && currentFunction && !isValueSearch) {
            ...
            if (CompoundStmt *CS = dyn_cast<CompoundStmt>(forStmt->getBody())) {
                ...
                std::string newFunctionName = currentFunction->getNameInfo().getName().getAsString() + "_innerLoop" +
                                              std::to_string(rewriterFunctionCount++);
...
class LoopExtractFrontendAction : public ASTFrontendAction {
public:
    LoopExtractFrontendAction() = default;
...
int main(int argc, const char **argv) {
    ...
    return Tool.run(newFrontendActionFactory<LoopExtractFrontendAction>().get());
}